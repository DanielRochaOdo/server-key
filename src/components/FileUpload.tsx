...
const FileUpload: React.FC<FileUploadProps> = ({ onSuccess, onCancel }) => {
  ...
  const mapHeader = (header: string): string | null => {
    ...
    if (norm.includes('data_pagamento') || norm.includes('pagamento') || norm.includes('payment')) return 'data_pagamento';
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('acessos').insert(rows);
