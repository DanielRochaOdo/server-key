...
const PessoalFileUpload: React.FC<PessoalFileUploadProps> = ({ onSuccess, onCancel }) => {
  ...
  const mapHeader = (header: string): string | null => {
    ...
    if (norm.includes('para_que_serve') || norm.includes('como_funciona')) return 'para_que_serve';
    if (norm.includes('ip') || norm.includes('url')) return 'ip_url';
    if (norm.includes('usuario') || norm.includes('login') || norm.includes('user')) return 'usuario_login';
    if (norm.includes('senha') || norm.includes('password')) return 'senha';
    if (norm.includes('observacao') || norm.includes('obs') || norm.includes('note')) return 'observacao';
    if (norm.includes('suporte') || norm.includes('contato') || norm.includes('support')) return 'suporte_contato';
    if (norm.includes('email') || norm.includes('mail')) return 'email';
    if (norm.includes('dia_pagamento') || norm.includes('pagamento') || norm.includes('payment')) return 'dia_pagamento';
  ...
  const parseFile = async () => {
    ...
    try {
      ...
      for (let i = 1; i < json.length; i++) {
        ...
        if (row.descricao) rows.push(row);
      }
    ...
    try {
      ...
      for (let i = 1; i < json.length; i++) {
        ...
        if (row.descricao) {
          ...
          rows.push({
            ...row,
            dia_pagamento: row.dia_pagamento ? parseInt(row.dia_pagamento) || null : null,
            user_id: user.id,
      ...
      const { error } = await supabase.from('pessoal').insert(rows);
    ...
    <div className="fixed inset-0 bg-neutral-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-neutral-200">
          <h2 className="text-xl font-semibold text-neutral-900">Importar Itens Pessoais</h2>
            ...
            <div>
              ...
              <div className="overflow-x-auto mb-4">
                <table className="min-w-full divide-y divide-neutral-200 border border-neutral-200 rounded-lg">
                  <thead className="bg-neutral-50">
                    <tr>
                      <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 uppercase">Descrição</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-neutral-500 uppercase">IP/URL</th>
                  ...
                  <tbody className="bg-white divide-y divide-neutral-200">
                    {preview.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-4 py-2 text-sm text-neutral-600">{row.ip_url || '-'}</td>
                        <td className="px-4 py-2 text-sm text-neutral-600">{row.usuario_login || '-'}</td>
              ...
              <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                <p className="text-sm text-blue-700">
                  <strong>Segurança:</strong> Todas as senhas serão automaticamente criptografadas antes de serem salvas no banco de dados.
                </p>
              </div>
...
export default PessoalFileUpload;
