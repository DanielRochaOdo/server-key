...
begin
  ...
  if new.tipo_movimentacao = 'entrada_compra' then
    ...
    new.origem_descricao := coalesce(
      nullif(upper(trim(new.origem_descricao)), ''),
      upper(coalesce(nullif(trim(pedido_row.item), ''), 'COMPRA'))
    );
    ...
    if coalesce(destino_normalizado, '') = '' or destino_normalizado in ('PARQUE TECNOLOGICO', 'ESTOQUE') then
      ...
      setor_key := regexp_replace(setor_key, '^MATRIZ', '', 'i');
      setor_key := trim(both ' -' from setor_key);
      setor_key := public.parque_normalizar_texto(replace(setor_key, '-', ' '));
