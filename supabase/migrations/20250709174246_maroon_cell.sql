...
-- Update win_users table policies if the table exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'win_users') THEN
    ...
    DROP POLICY IF EXISTS "Allow select for authenticated" ON win_users;
    DROP POLICY IF EXISTS "Allow insert for authenticated" ON win_users;
    DROP POLICY IF EXISTS "Allow update for authenticated" ON win_users;
    DROP POLICY IF EXISTS "Allow delete for authenticated" ON win_users;
    ...
    ELSE
      ...
      EXECUTE 'CREATE POLICY "Users with win_users module can manage data"
        ON win_users
        FOR ALL
        TO authenticated
        USING (has_module_access(''win_users''))
