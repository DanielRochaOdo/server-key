/*
  ...
  1. Users Table Policies
    - Allow public insert (needed for Edge Functions)
    - Allow users to read their own data
    - Allow users to update their own data
    - Multiple select policies for different scenarios

  2. Module-based Policies
    - Update all module tables with proper RLS
    - Ensure users can only access data for modules they have permission
    - Maintain user_id ownership checks

  3. Security
    - All policies check users.is_active = true
    - Module access controlled via users.modules array
...
-- Allow users to update their own record by ID
